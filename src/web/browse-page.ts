...
<p class="note">The approval card lives in Aisle's page, never inside the browser. Buying happens in a separate, read-only Steel session.</p>
</aside></main>
<script>
...
async function tick(){
  const s = await (await fetch("/api/browse/state")).json();
  if (!s.active) setViewer(null, null);
  else if (s.workerDebugUrl) setViewer(s.workerDebugUrl, "Aisle is buying in a separate browser");
