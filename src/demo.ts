...
import { signMandate } from "./fast-lane/guards.js";
import { MockWebMcpVendor } from "./mock/mock-vendor.js";
import type { FastLaneRequest, PurchaseMandate } from "./types.js";

const PROVIDER = "mock-image-api";
const ORIGIN = "https://api.mock-image-api.test";
// Stands in for the shared secret the approval/policy layer would hold in a
// real deployment. Never hardcode this outside a demo.
const DEMO_MANDATE_SECRET = "demo-only-shared-secret";

function buildRequest(): FastLaneRequest {
  const expiresAt = new Date(Date.now() + 5 * 60 * 1000).toISOString();
  const unsignedMandate: Omit<PurchaseMandate, "signature"> = {
    mandateId: "mnd_001",
  ...
  return {
    ...
    mandate: {
      ...unsignedMandate,
      signature: signMandate(unsignedMandate, DEMO_MANDATE_SECRET),
    },
  };
}
...
async function main(): Promise<void> {
  ...
  const result = await runFastLane(buildRequest(), {
    session: vendor,
    mandateSecret: DEMO_MANDATE_SECRET,
