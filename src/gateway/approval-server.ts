...
<p class="note">After approval Aisle buys in a Steel browser on the vendor's billing page and pays for you, then checks the balance rose and retries the blocked call. If the vendor asks you to sign in, do it in that browser; Aisle waits, then carries on.</p>
</section>
...
function renderViewer(s){
  const key = s.live ? "live:"+s.live.sessionId+":"+(s.live.interactive?1:0) : s.steel ? "done:"+s.steel.sessionId : null;
  ...
  if (s.live){
    ...
    if (s.live.interactive){
      const t=document.createElement("p"); t.className="livebadge";
      t.textContent = s.live.takeoverReason==="LOGIN_REQUIRED"
        ? "Aisle needs you: sign in to the vendor in the browser below. Aisle continues the purchase as soon as you're in."
        : "Aisle needs you: clear the verification (" + (s.live.takeoverReason||"challenge") + ") in the browser below. Aisle then checks the balance.";
      v.append(t);
    }
