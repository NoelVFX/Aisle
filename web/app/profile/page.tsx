...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    try {
      const saved = JSON.parse(localStorage.getItem(PROFILE_STORAGE_KEY) || "null") as { tags?: string[]; context?: string } | null;
      if (saved) { setTags(Array.isArray(saved.tags) ? saved.tags : []); setContext(typeof saved.context === "string" ? saved.context : ""); }
    } catch { /* ignore */ }
    fetch("/api/profile", { cache: "no-store" })
      .then((r) => (r.ok ? r.json() as Promise<{ user?: { email?: string }; profile?: { profile_tags?: string[] | null; profile_context?: string | null } | null }> : null))
      .then((d) => {
        if (d?.user?.email) setEmail(d.user.email);
        if (d?.profile) { setTags(d.profile.profile_tags ?? []); setContext(d.profile.profile_context ?? ""); }
      })
      .catch(() => {})
    ...
    <>
      ...
      <div style={{ minHeight: "100dvh", display: "grid", placeItems: "center", padding: 24, position: "relative", zIndex: 1 }}>
        <div className="card card-pad" style={{ width: "min(560px, 100%)", display: "flex", flexDirection: "column", gap: 18, boxShadow: "0 24px 80px rgba(0,0,0,.35)" }}>
          ...
          {loaded
            ? <ProfileEditor initialTags={tags} initialContext={context} heading="Tell Aisle how to shop for you" />
