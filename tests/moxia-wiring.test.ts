...
function inProcess() {
  ...
  const g = createGateway({ upstreams: loadUpstreams(undefined, {}), steel: fastSteel, mockVendor: mock, safeBlockMs: 30, pollMs: 5, publicUrl: () => "http://x", mandateSecret: SECRET, log: () => {} });
  return { g, mock };
}

async function open(g: ReturnType<typeof createGateway>) {
  const first = json((await g.callTool("mockvendor__generate_image", { prompt: "hero" }, { taskId: "t" })) as never);
...
describe("recovery session state machine", () => {
  ...
  it("an unverified purchase ends PURCHASE_UNKNOWN, an explicit failure PURCHASE_FAILED", async () => {
    const make = (code: string) => {
      ...
      const fetchImpl: FetchLike = async () => ({ ok: false, status: 402, headers: { forEach: () => {} }, text: async () => JSON.stringify({ code: "insufficient_credits", required_credits: 1067 }) });
      const purchaser: SteelPurchaser = { async purchase() { throw Object.assign(new Error(code), { code }); } };
      return createGateway({ upstreams, steel: fastSteel, purchaser, fetchImpl, safeBlockMs: 30, pollMs: 5, publicUrl: () => "http://x", mandateSecret: SECRET, log: () => {} });
    };
