...
const fastDeps = (session: MockWebMcpVendor, store: InMemoryIdempotencyStore) => ({
  session,
  store,
...
describe("5. the slow lane keeps an unverified purchase blocking", () => {
  it("does not let a new mandate buy again after a failed Gate 2", async () => {
    const site = new MockVendorSite({ provider: "mock-slow-vendor", origin: "https://shop.mock-slow-vendor.test", dropCredits: true });
    const store = new InMemoryIdempotencyStore();
